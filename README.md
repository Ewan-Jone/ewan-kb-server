...
- **多 KB 支持** — 同时服务多个知识库。每个 KB 启动时预加载图谱和 BM25 索引，查询时通过 `kb` 参数指定目标。内部通过 EWANKB_DIR 环境变量切换，确保多 KB 环境下查询指向正确目录。
...
返回渲染后的文本摘要。`verbose=True` 时返回结构化 JSON（含 `matched_start_nodes`、`nodes`、`edges` 等字段）。
